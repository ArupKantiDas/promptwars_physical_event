 ...
 * Covers: Power-of-Two-Choices sampling, scoring, proximity weighting, edge cases.
 */
...
describe('computeGateScore', () => {
  ...
  it('correctly weights queue load vs proximity', () => {
    // Gate A: low queue but far away → scoreA = (10/60)*(1/0.2) = 0.833
    // Gate B: higher queue but very close → scoreB = (30/60)*(1/0.9) = 0.555
...
describe('assignGate', () => {
  ...
  it('returns null when gateStates is empty', () => {
    const mappings: SectionZoneMapping[] = [{ zoneId: 'zone-a', gateIds: ['g1'], proximityScore: 0.8 }];
  ...
  it('returns null when gateIds lists are all empty', () => {
    const mappings: SectionZoneMapping[] = [{ zoneId: 'zone-z', gateIds: [], proximityScore: 0.9 }];
    const states = new Map([['g1', makeState()]]);
    expect(assignGate('201', mappings, states)).toBeNull();
  });

  it('assigns the only eligible gate when one exists', () => {
    const mappings: SectionZoneMapping[] = [{ zoneId: 'zone-b', gateIds: ['gate-1'], proximityScore: 0.9 }];
    const states = new Map([['gate-1', makeState()]]);
  ...
  it('picks the better gate out of two sampled candidates', () => {
    vi.mocked(Math.random).mockRestore();

    const mappings: SectionZoneMapping[] = [{
      zoneId: 'zone-a',
      gateIds: ['empty', 'congested'],
      proximityScore: 1.0,
    }];
  ...
  it('returns a gateId and zoneId from the winning candidate', () => {
    const mappings: SectionZoneMapping[] = [{ zoneId: 'zone-c', gateIds: ['g-box'], proximityScore: 0.8 }];
